...
async create(dto: CreateProductDto, file: Express.Multer.File): Promise<Product> {
 ...
 // ✅ MÉTODO findAll OPTIMIZADO
  async findAll(filters: { brand?: string; category?: string; game?: string; page?: number; limit?: number; search?: string }) {
    const { page = 1, limit = 10, search, brand, game, category } = filters;
    ...
    if (brand) query.andWhere('brand.name = :brandName', { brandName: brand });
