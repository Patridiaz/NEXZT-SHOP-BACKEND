...
export class AuthService {
  ...
  async validateUser(email: string, password: string): Promise<Omit<User, 'password'> | null> {
    ...
    if (user && !user.isActive) {
      // Si el usuario está desactivado, no permitimos el login
      return null;
    }
  ...
  async registerFromGuest(dto: RegisterFromGuestDto) {
    ...
    this.mailService.sendWelcomeEmail(newUser.email, newUser.name).catch(() => { });
  ...
  async register(dto: CreateUserDto): Promise<Omit<User, 'password'>> {
    ...
    // Enviar correo de bienvenida
    this.mailService.sendWelcomeEmail(savedUser.email, savedUser.name).catch(() => { });
